...
Latest release **0.5.8** (2026-09-16) · **9** changes in development · [full changelog](CHANGELOG.en.md)

<details open>
<summary><b>2026-09-17</b> · Unreleased · 1 added · 2 style · 4 fixed</summary>
...
- In the open island, "N not updating" in the footer did nothing when clicked and gave no hint of which providers or why, and a refresh whose reads came straight back showed its spinner for a blink, so the button looked dead. Hover over the note or click it and it lists each provider that is not updating: its mark, its name and what its last refresh said, with how old the numbers are where older ones are still shown; click one to open Settings → Providers at its row. After Refresh the spinner stays for at least 0.8 seconds, then the note says "All up to date" or "Refreshed · N not updating" for about 2 seconds.
