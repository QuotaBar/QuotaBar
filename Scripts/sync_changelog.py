#!/usr/bin/env python3
"""Puts the changelogs where people look and draws the commit calendar.
...
Run it after every CHANGELOG edit. The websites live in their own repositories next
to this one (../quota.bar and ../quota.run); they read the changelogs, ProviderID and
the app's logos from here when they are built.
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Sync the changelogs into the READMEs and draw the activity chart.")
    ...
    _, releases_zh = parse(pathlib.Path(args.changelog).read_text(encoding="utf-8"))
    _, releases_en = parse(pathlib.Path(args.changelog_en).read_text(encoding="utf-8"))
    changed = []
