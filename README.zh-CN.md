...
| Kimi Code | Kimi Code 应用 / CLI 登录（`~/.kimi-code/credentials`，按 Kimi Code 自己的续期锁续期）或 Kimi Code API Key → `api.kimi.com` / `api.kimi.ai` `/coding/v1/usages`，自动识别国内版 / 国际版；或用 `kimi-auth` cookie 读 `kimi.com` / `kimi.ai` 计费网关 | 自动 / 手动填写 API Key / Cookie |
