...
最新版本 **0.5.8**（2026-09-16） · 开发中 **13** 项改动尚未发布 · [完整更新日志](CHANGELOG.md)

<details open>
<summary><b>2026-09-19</b> · 未发布 · 修复 1</summary>

**修复**

- Kimi Code 卡片把用满的额度显示成「已用 0%」：Kimi 的用量接口同时返回新旧两种数字，新格式的额度比例报 0，旧格式的计数却是 5 小时窗口 100 / 100、本周 21 / 100。以前只要有新格式就只看它；现在同一个窗口两种都读，取已用更多的那个，并显示「已用 / 上限」。

</details>

<details>
<summary><b>2026-09-17</b> · 未发布 · 新增 3 · 样式 2 · 修复 5</summary>
...
- 刷新失败、仍在显示旧读数的服务商，在刘海岛、下拉面板的卡片和边缘停靠条的卡片上，服务状态旁边会显示橙色的「● 未能更新」：鼠标悬停可以看到原因和数据是多久前的，点击打开设置里这个服务商。刘海岛上这类服务商的数字和进度条变暗，下拉面板和停靠条卡片上的进度条也变暗。旧读数里重置时间已经过去的窗口，「已到重置时间」改用橙色显示，不再像是当前数据；重置后的用量不会凭空估算。
- 下拉面板卡片里「显示的是 X 前的数据」下面直接写出刷新失败的原因，不用再把鼠标悬停上去才能看到。

**修复**

- 刘海岛展开面板底栏的「N 个未能更新」点了没有反应，也看不出是哪些服务商、为什么没更新；点刷新按钮时读取很快就返回，转圈一闪而过，看起来像没点到。现在把鼠标在这行字上停一下或点一下，上方会列出每个未能更新的服务商：图标、名称和上次刷新的出错原因，还显示着旧读数的会注明数据是多久前的；点其中一个直接打开设置 → 服务商里它的那一行。点开的列表会一直开着，直到再点一下；列表开着时点这行字会把它收起。点刷新后转圈至少持续 0.8 秒，结束后这行字显示约 2 秒「已全部更新」或「已刷新 · N 个未能更新」。
- 刷新进行中时，上次刷新失败、又没有任何读数的服务商会被当成「加载中」而不是失败：下拉面板和边缘停靠条上它的卡片显示「加载中…」，出错原因不见了；刘海岛和下拉面板底栏的圆点也从橙色变成绿色，直到读取返回。现在新的读取返回之前，它保留出错原因，也仍算作未能更新。
- Claude Code 在这台 Mac 上退出登录后（钥匙串里的条目还在，但登录令牌被清空），Claude 卡片提示「尚未配置。运行一次 `claude` 并登录以生成 OAuth 会话。」，像是从来没有登录过。现在会说明 Claude Code 已在这台 Mac 上退出登录，请在终端运行 `claude`，再输入 /login 登录；设置 → 服务商里 Claude 的状态显示「需登录」，展开后的提示也说明是已退出登录。套餐（如 Max 20x）照常读取。
...
- 边缘停靠条收起时的把手没有跟随选定的服务商：在停靠条上选了 Antigravity（圆环下有绿点），把手仍按停靠条上最快用完的服务商（比如 Cursor）显示满格红条并闪烁提醒。现在把手的进度、颜色和低额度闪烁都跟随选定的服务商，它额度充足就不闪；没有选定服务商时，才看停靠条上最紧张的那个。

**删除**

- 去掉升级后自动弹出「分享用量卡片」窗口：以前每升级到一个新版本，第一次启动时只要这周有用量就会自己打开一次，连续升级时每次都弹，容易误以为出了问题。分享卡片仍可以从下拉面板、刘海岛总览页右上角的分享图标，以及设置 → 用量统计里打开。
